...
function extractBongda($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  if (sapoText && sapoText.length > 20) {
    ...
    const normalizedDesc = (sapoText as string).replace(/\s+/g, " ").trim();
    contentClone.find("h2, p").each((_, el) => {
      const $el = $(el);
      if ($el.text().replace(/\s+/g, " ").trim() === normalizedDesc) {
        $el.remove();
        return false;
      }
    });
...
function extractVietnameseGeneric(
  ...
  // Fallback to description metadata if no sapo found
  if (!sapoText && description && description.length > 20) {
    sapoText = description.trim();
    pushUnique(paragraphs, seenP, sapoText);

    // Deduplicate: Find and remove any heading or paragraph that exactly matches the description
    const normalizedDesc = (sapoText as string).replace(/\s+/g, " ").trim();
    contentClone.find("h2, p").each((_, el) => {
      const $el = $(el);
      const elText = $el.text().replace(/\s+/g, " ").trim();
      if (elText === normalizedDesc) {
        $el.remove();
