...
function extractBongda($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  if (sapoText && sapoText.length > 20) {
    ...
    // Deduplicate from htmlContent by removing any h2 or p that matches sapoText
    ...
    // Prepend the sapo right after deduplication
...
function extractBongdaplus(
  ...
  const $sapo = container.find(".sapo").first().length ? container.find(".sapo").first() : $(".detail-sapo, .sapo").first();
