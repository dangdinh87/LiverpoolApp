...
function extractBongda($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  if (sapoText && sapoText.length > 20) {
    // Deduplicate from html content if fallback was used
    const normalizedSapo = sapoText.replace(/\s+/g, " ").trim();
...
function extractBongdaplus($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  let sapoText =
    description?.trim() ||
    $('meta[property="og:description"]').attr("content")?.trim();
  const $sapo = container.find(".sapo").first().length
    ? container.find(".sapo").first()
    : $(".detail-sapo, .sapo").first();
...
function extract24h($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  let sapoText =
    description?.trim() ||
    $('meta[property="og:description"]').attr("content")?.trim();
  const $sapo = $("#article_sapo").first();
  const sapo = $sapo.text().trim();

  // Create clone here since we modify it in sapo logic for standardization
  const clone = container.clone();

  if (sapo && sapo.length > 20) {
    sapoText = sapo;
    clone.find("#article_sapo").first().remove();
  }
...
function extractVietnamvn($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  const sapoText =
    description?.trim() ||
    $('meta[property="og:description"]').attr("content")?.trim();
  // Vietnam.vn doesn't have a clear sapo element, rely on fallback
...
function extractWebthethao($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  const sapoText =
    description?.trim() ||
    $('meta[property="og:description"]').attr("content")?.trim();
  // Webthethao doesn't have a clear sapo element, rely on fallback
