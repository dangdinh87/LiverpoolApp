...
function resolveImageSrc($el: cheerio.Cheerio<any>, baseUrl: string): string | undefined {
  const helper = (path?: string) => {
    if (!path || path.startsWith("data:image")) return undefined;
  ...
  const dataOriginal = helper($el.attr("data-original"));
  if (dataOriginal) return dataOriginal;

  const dataSrc = helper($el.attr("data-src"));
  ...
  if (dataSrcset) {
    ...
    const resolvedHighestRes = helper(highestRes);
    if (resolvedHighestRes) return resolvedHighestRes;
  }

  return helper($el.attr("src"));
}
