...
function extractVietnameseGeneric(
  ...
  // Fallback to description metadata if sapo extraction fails
  if (!sapoText && description && description.length > 20) {
    const trimmedDesc = description.trim();
    pushUnique(paragraphs, seenP, trimmedDesc);
    sapoText = trimmedDesc;

    // Prevent duplication by removing matching h2 or p in the cloned container
    contentClone.find("h2, p").each((_, el) => {
      if ($(el).text().trim() === trimmedDesc) {
        $(el).remove();
      }
    });
  }
