...
function resolveImageSrc($el: cheerio.Cheerio<any>, baseUrl?: string): string | undefined {
  const parseUrl = (urlStr: string | undefined): string | undefined => {
    if (!urlStr || urlStr.startsWith("data:image")) return undefined;
  ...
  const dataOriginal = parseUrl($el.attr("data-original"));
  if (dataOriginal) return dataOriginal;

  const dataSrc = parseUrl($el.attr("data-src"));
  ...
  if (dataSrcset) {
    ...
    const resolvedSrcset = parseUrl(highestRes);
    if (resolvedSrcset) return resolvedSrcset;
  }

  const src = parseUrl($el.attr("src"));
  if (src) return src;
  ...
  container.find("img").each((_, el) => {
    ...
    if (!realSrc) {
      ...
      if ($parentFigure.length > 0) {
        let metaUrl = $parentFigure.find("meta[itemprop='url']").attr("content");
        if (metaUrl) {
          if (!metaUrl.startsWith("http") && baseUrl) {
            try {
              metaUrl = new URL(metaUrl, baseUrl).toString();
            } catch {
              metaUrl = undefined;
            }
          }
  ...
  if (opts?.htmlContent !== false) {
    ...
    if (opts?.sapoSelector) {
      ...
      if (sapo && sapo.length > 20) {
        htmlContainer = container.clone();
        htmlContainer.prepend(`<p class="sapo"><strong>${sapo}</strong></p>`);
      }
    }
    htmlContent = buildHtmlContent(htmlContainer, $, url) || undefined;
  }
...
function extractVnexpress($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  const htmlContent = buildHtmlContent(htmlContainer, $, url) || undefined;
