...
function extractVietnameseGeneric(
  ...
  if (!sapoText && description && description.length > 20) {
    ...
    // Remove exactly matching h2 or p tags to avoid duplication
...
function extractZnews($: cheerio.CheerioAPI, url: string): ArticleContent {
  const container = $(".the-article-body, .article-content, article, [role=main]").first();
  container.find(".inner-article, table.article, .notebox, .the-article-tags, .sidebar, .topics, .the-article-credit, header.the-article-header, header").remove();

  const article = extractVietnameseGeneric($, url, ".the-article-body, .article-content, article, [role=main]", "ZNews", { sapoSelector: ".the-article-summary" });
  article.isThinContent = article.paragraphs.length <= 2;
  return article;
}
