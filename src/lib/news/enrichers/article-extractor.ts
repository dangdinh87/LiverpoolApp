...
function resolveImageSrc($el: cheerio.Cheerio<any>, baseUrl: string): string | undefined {
  const getAbsoluteUrl = (srcStr: string | undefined) => {
    if (!srcStr) return undefined;
    if (srcStr.startsWith("data:")) return undefined;
    try {
      return new URL(srcStr, baseUrl).href;
    } catch {
      return undefined;
    }
  };
...
function extractVietnamese(
  ...
  const sapoSelectors = "h2.sapo, .sapo, p.description, .detail-sapo";
  const $sapo = container.find(sapoSelectors).first();
  const sapoText = $sapo.text().trim();
  if (sapoText && sapoText.length > 20) {
    pushUnique(paragraphs, seenP, sapoText);
    contentClone.find(sapoSelectors).remove();
    contentClone.prepend(`<p class="sapo"><strong>${sapoText}</strong></p>`);
  }
