...
function extract24h($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  if (!sapo && description && description.length > 20)
...
function extractVietnameseGeneric(
  ...
  if (opts?.sapoSelector) {
    const $sapo = container.find(opts.sapoSelector).first();
  ...
  // Fallback to description metadata if sapo selector yields nothing
