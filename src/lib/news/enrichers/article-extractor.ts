...
function extractBongda($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  if (sapoText && sapoText.length > 20) {
    // Remove the first exact match of description from the clone to deduplicate
    contentClone.find("h2, p").each((_, el) => {
      const text = $(el).text().replace(/\s+/g, " ").trim();
      const descText = sapoText.replace(/\s+/g, " ").trim();
      if (text === descText) {
        $(el).remove();
...
function extractBongdaplus(
  ...
  if (sapoText && sapoText.length > 20) {
    ...
    contentClone.find("h2, p").each((_, el) => {
      const text = $(el).text().replace(/\s+/g, " ").trim();
      const descText = description.replace(/\s+/g, " ").trim();
      if (text === descText) {
        $(el).remove();
        return false;
      }
