...
function extractBongdaplus(
  ...
  if (sapoText && sapoText.length > 20) {
    ...
    contentClone.prepend(`<p class="sapo"><strong>${sapoText}</strong></p>`);
  } else if (!sapoText && description && description.length > 20) {
...
function extract24h($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  } else if (!sapo && description && description.length > 20) {
  ...
  if (sapo && sapo.length > 20) {
    ...
    // Explicitly remove exactly matched fallback text to prevent duplication if fallback was used
    clone.find("h2, p").each((_, el) => {
      if ($(el).text().trim() === sapo) {
        $(el).remove();
      }
    });
...
function extractVietnameseGeneric(
  ...
  if (!sapoText && description && description.length > 20) {
    ...
    // Explicitly remove any h2 or p that exactly matches the fallback text to prevent duplication
    contentClone.find("h2, p").each((_, el) => {
      if ($(el).text().trim() === sapoText) {
        $(el).remove();
      }
