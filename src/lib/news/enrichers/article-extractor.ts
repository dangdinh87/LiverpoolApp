...
function extractZnews($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  // Remove related/junk elements embedded inside the container before passing to generic
  ...
  const result = extractVietnameseGeneric(
    $,
    url,
    ".the-article-body, .article-content, article, [role=main]",
    "ZNews",
  ...
  result.isThinContent = result.paragraphs.length <= 2;
  return result;
}
