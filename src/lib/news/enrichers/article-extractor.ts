...
function extract24h($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  container.find("p, figcaption").each((_, el) => {
    ...
    if (cleanText.length > 30 && !junkPattern.test(cleanText) && !matchWidgetPattern.test(cleanText)) {
      if (sapoText && cleanText === sapoText.replace(/\s+/g, " ").trim()) return;
  ...
  if (sapoText && sapoText.length > 20) {
    const normalizedSapo = sapoText.replace(/\s+/g, " ").trim();
    clone.find("h2, p, #article_sapo").each((_, el) => {
      const text = $(el).text().replace(/\s+/g, " ").trim();
      if (text === normalizedSapo || $(el).attr("id") === "article_sapo") {
        $(el).remove();
      }
    });
...
function extractVietnameseGeneric(
  ...
  // Deduplicate sapo from container
  if (sapoText) {
    const normalizedSapo = sapoText.replace(/\s+/g, " ").trim();
    contentClone.find("h2, p").each((_, el) => {
      const text = $(el).text().replace(/\s+/g, " ").trim();
