...
function extractBongda($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  if (sapoText && sapoText.length > 20) {
    // Remove exactly matching element in the clone to prevent duplicate
    let matchRemoved = false;
    contentClone.find("h2, p").each((_, el) => {
      if (matchRemoved) return;
...
function extractBongdaplus(
  ...
  // Fallback sapo behavior using description metadata
...
function extract24h($: cheerio.CheerioAPI, url: string): ArticleContent {
  ...
  if (sapoText && sapoText.length > 20) pushUnique(paragraphs, seenP, sapoText);
  ...
  if (sapoText && sapoText.length > 20) {
    ...
    // Remove exactly matching element in the clone to prevent duplicate if falling back
    let matchRemoved = false;
