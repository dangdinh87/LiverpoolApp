...
export default async function FixtureDetailPage({ params }: PageProps) {
  ...
  const [events, lineups, providerStats, espnDetail, prevFixtures] =
    await Promise.all([
  ...
  const opponentName =
    teams.home.id === LFC_ID ? teams.away.name : teams.home.name;
  ...
  for (const key of STAT_KEYS) {
    try {
      statLabels[key] = tMatch(`statLabels.${key}`);
    } catch {
      statLabels[key] = key;
    }
  }

  const matchStats = espnDetail?.stats?.length
    ? espnDetail.stats
    : providerStats;
  const homeStats = matchStats.find(
    (s) =>
      s.team.id === teams.home.id ||
      s.team.name
        .toLowerCase()
        .includes(teams.home.name.toLowerCase().split(" ")[0].toLowerCase()),
  );
  const awayStats = matchStats.find(
    (s) =>
      s.team.id === teams.away.id ||
      s.team.name
        .toLowerCase()
        .includes(teams.away.name.toLowerCase().split(" ")[0].toLowerCase()),
  );

  // Match lineups by team ID first, then by name (IDs differ across FDO/ESPN providers)
  const homeLineup =
    lineups.find((l) => l.team.id === teams.home.id) ??
    lineups.find((l) =>
      l.team.name
        .toLowerCase()
        .includes(teams.home.name.toLowerCase().split(" ")[0].toLowerCase()),
  ...
  const awayLineup =
    lineups.find((l) => l.team.id === teams.away.id) ??
    lineups.find((l) =>
      l.team.name
        .toLowerCase()
        .includes(teams.away.name.toLowerCase().split(" ")[0].toLowerCase()),
  ...
  const venue =
    espnDetail?.venue ??
    ([f.venue.name, f.venue.city].filter(Boolean).join(", ") || null);
  ...
  const resolvedGoals = goalEvents.map((e) => {
    ...
    const homeMatch =
      homeName.includes(name) || name.includes(homeName.split(" ")[0]);
    const awayMatch =
      awayName.includes(name) || name.includes(awayName.split(" ")[0]);
    const resolvedId = homeMatch
      ? teams.home.id
      : awayMatch
        ? teams.away.id
        : 0;
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <div className="bg-stadium-surface border border-stadium-border p-5 sm:p-6 mb-4 relative overflow-hidden">
          ...
          {league.logo &&
            (() => {
              const isDarkLogo =
                league.name.includes("Premier") ||
                league.name.includes("Champions");
              return (
                <div className="absolute -right-6 top-1/2 -translate-y-1/2 w-64 h-64 opacity-[0.10] -rotate-12 pointer-events-none select-none">
                  <Image
                    ...
                    fill
                    sizes="256px"
                    className={`object-contain ${isDarkLogo ? "brightness-0 invert" : ""}`}
                    loading="lazy"
                  />
                </div>
              );
          ...
          {isLive && (
            <div className="absolute left-0 top-0 bottom-0 w-1 bg-lfc-red animate-pulse" />
          )}
          ...
          <div className="relative flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              {league.round && (
                <>
                  ...
                  <span className="text-stadium-muted font-barlow text-sm">
                    {league.round}
                  </span>
                </>
              )}
            ...
            {isFinished && result !== "NS" && (
              <span className="inline-flex items-center gap-1.5">
                ...
                <span
                  className={cn(
                    "text-xs font-bebas tracking-widest leading-none",
            ...
            {isLive && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-lfc-red/15 border border-lfc-red/30">
                ...
                <span className="text-lfc-red text-[11px] font-bebas tracking-wider leading-none">
                  {tMatch("live")} {f.status.elapsed}&apos;
                </span>
              </span>
            )}
          </div>
          ...
          <div className="relative flex items-center justify-center gap-3 sm:gap-6">
            ...
            <div className="flex items-center gap-2.5 flex-1 justify-end">
              <div
                className={cn(
                  "text-right",
                  teams.home.id === LFC_ID ? "text-white" : "text-white/70",
                )}
              >
                <span className="font-inter font-semibold text-sm sm:text-base block">
                  {teams.home.name}
                </span>
                {homeLineup && (
                  <span className="font-barlow text-stadium-muted text-[10px]">
                ...
                {isFinished && homeGoals.length > 0 && (
                  <div className="mt-1 space-y-0.5">
                    {homeGoals.map((g, i) => (
                      <p
                        key={i}
                        className="text-[11px] font-inter text-stadium-muted"
                      >
                        <span className="text-white/80">{g.player.name}</span>
                        {g.time.elapsed ? (
                          <span className="text-lfc-gold ml-1">
                            {g.time.extra
                              ? `${g.time.elapsed}+${g.time.extra}'`
                              : `${g.time.elapsed}'`}
                          </span>
                        ) : null}
                        {g.detail.includes("Penalty") && (
                          <span className="text-stadium-muted ml-0.5">(P)</span>
                        )}
                        {g.detail.includes("Own") && (
                          <span className="text-red-400 ml-0.5">(OG)</span>
                        )}
                      </p>
                    ))}
                  </div>
              ...
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 shrink-0">
                <Image
                  src={teamLogo(teams.home.id, teams.home.logo)}
                  alt={teams.home.name}
                  fill
                  sizes="48px"
              ...
              {isFinished || isLive ? (
                <>
                  <div className="flex items-center justify-center gap-2">
                    <span
                      className={cn(
                        "font-bebas text-4xl sm:text-5xl tracking-wider leading-none",
                        teams.home.winner ? "text-white" : "text-white/50",
                      )}
                    >
                      {goals.home}
                    </span>
                    <span className="text-stadium-muted font-bebas text-3xl leading-none">
                      :
                    </span>
                    <span
                      className={cn(
                        "font-bebas text-4xl sm:text-5xl tracking-wider leading-none",
                        teams.away.winner ? "text-white" : "text-white/50",
                      )}
                    >
                      {goals.away}
                    </span>
                  ...
                  {isFinished && hasHt && (
                    <p className="text-stadium-muted text-[10px] font-inter mt-1">
                      {tMatch("halftimeShort")} {htHome} – {htAway}
                    </p>
                  )}
                ...
                <span className="font-bebas text-4xl text-white tracking-wider leading-none block">
                  {timeStr}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2.5 flex-1">
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 shrink-0">
                <Image
                  src={teamLogo(teams.away.id, teams.away.logo)}
                  alt={teams.away.name}
                  fill
                  sizes="48px"
                  className="object-contain"
              ...
              <div
                className={cn(
                  teams.away.id === LFC_ID ? "text-white" : "text-white/70",
                )}
              >
                <span className="font-inter font-semibold text-sm sm:text-base block">
                  {teams.away.name}
        ...
        {(() => {
          ...
          const lineupLabel = isConfirmed
            ? tDetail("lineupsConfirmed")
            : tDetail("lineupsPredicted");
                    ...
                    <div className="bg-stadium-surface border border-stadium-border p-4 sm:p-5">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {homeLineup && (
                          <LineupSection
                            lineup={homeLineup}
                            subsLabel={tDetail("substitutes")}
                            coachLabel={tDetail("coach")}
                          />
                        )}
                        {awayLineup && (
                          <LineupSection
...
function TimelineRow({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex items-center gap-1.5 flex-1 flex-wrap",
      ...
      <div className="flex items-center gap-1 shrink-0 w-[60px] justify-center">
        ...
        {minute && (
          <span className="font-barlow text-stadium-muted text-[11px] font-semibold">
            {minute}
          </span>
        )}
      </div>
...
function EventIcon({ type, detail }: { type: string; detail: string }) {
  if (type === "Goal") {
    ...
    return (
      <span className={cn("text-sm", isOwn ? "text-red-400" : "text-white")}>
        {isPen ? "⚽P" : isOwn ? "⚽OG" : "⚽"}
      </span>
    );
  }
  if (type === "Card") {
    return (
      <span
        className={cn(
          "inline-block w-3 h-4 rounded-[1px]",
          detail.includes("Red") ? "bg-red-500" : "bg-yellow-400",
        )}
      />
    );
  }
  ...
  return (
    <div className="space-y-3">
      {statTypes.map((type) => {
        ...
        return (
          <div key={type}>
            <p className="text-stadium-muted text-[11px] font-inter text-center mb-1">
              {statLabels[type] ?? type}
            </p>
            <div className="flex items-center gap-2">
              <span
                className={cn(
                  "font-inter text-sm font-semibold w-14 text-right",
                  hWins ? "text-white" : "text-stadium-muted",
                )}
              >
                {hStr}
              </span>
                ...
                <div className="flex-1 flex justify-end bg-stadium-surface2 rounded-l-sm overflow-hidden">
                  <div
                    className={cn(
                      ...
                      hWins ? "bg-lfc-red" : "bg-stadium-muted/50",
                    )}
                    style={{ width: `${hPct}%` }}
                  />
                </div>
                <div className="flex-1 bg-stadium-surface2 rounded-r-sm overflow-hidden">
                  <div
                    className={cn(
                      ...
                      aWins ? "bg-sky-500" : "bg-stadium-muted/50",
                    )}
                    style={{ width: `${aPct}%` }}
                  />
                </div>
              ...
              <span
                className={cn(
                  "font-inter text-sm font-semibold w-14 text-left",
                  aWins ? "text-white" : "text-stadium-muted",
                )}
              >
                {aStr}
              </span>
    ...
    <div>
      ...
      <div className="flex items-center gap-2.5 mb-3 pb-2 border-b border-stadium-border/50">
        {(lineup.team.id === LFC_ID || lineup.team.logo) && (
          <div className="relative w-5 h-5 shrink-0">
            <Image
              src={
                lineup.team.id === LFC_ID
                  ? "/assets/lfc/crest.webp"
                  : lineup.team.logo
              }
              alt=""
              fill
        ...
        <span className="font-bebas text-lfc-gold text-base tracking-wider ml-auto">
          {lineup.formation}
        </span>
      </div>
      ...
      <div className="space-y-0.5 mb-4">
        {lineup.startXI.map(({ player: p }) => (
          <div
            key={p.id}
            className="flex items-center gap-3 py-1.5 hover:bg-stadium-surface2/50 px-1 -mx-1 transition-colors"
          >
      ...
      {lineup.substitutes.length > 0 && (
        <div className="border-t border-stadium-border/50 pt-3">
          <p className="font-barlow text-stadium-muted text-xs uppercase tracking-wider mb-2 font-semibold">
            {subsLabel}
          </p>
          <div className="space-y-0.5">
            {lineup.substitutes.map(({ player: p }) => (
              <div
                key={p.id}
                className="flex items-center gap-3 py-1 px-1 -mx-1"
              >
                ...
                <span className="text-stadium-muted font-inter text-sm">
                  {p.name}
                </span>
              </div>
            ))}
      ...
      {lineup.coach.name && (
        <div className="border-t border-stadium-border/50 pt-3 mt-3 flex items-center gap-2.5">
          ...
          <div>
            <p className="text-[10px] font-barlow text-stadium-muted uppercase tracking-wider">
              {coachLabel}
            </p>
