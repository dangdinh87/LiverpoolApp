...
export function detectWebSearch(message: string): boolean {
  ...
  return (
    WEAK_SIGNALS.reduce((acc, kw) => acc + (lower.includes(kw) ? 1 : 0), 0) >= 2
  );
}
  ...
  return (
    <div className="flex items-start gap-3 px-2 py-3 max-w-3xl mx-auto w-full animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="relative shrink-0">
        ...
        {/* Pulse ring */}
        <div
          className="absolute inset-0 rounded-full bg-lfc-red/30 animate-ping"
      ...
      <div className="flex-1 pt-1">
        ...
        <div className="space-y-2">
          <div className="h-3 rounded-full overflow-hidden bg-muted/30">
            <div className="h-full w-2/3 rounded-full bg-gradient-to-r from-muted/0 via-muted/60 to-muted/0 animate-shimmer" />
          </div>
          <div className="h-3 rounded-full overflow-hidden bg-muted/30">
            <div
    ...
    >
      ...
      <div className="flex-1 space-y-2 pt-1">
        {widths.map((w, i) => (
          <div
            key={i}
            className={`h-3.5 rounded-full overflow-hidden bg-muted/40 ${w}`}
          >
            <div
  ...
  return (
    ...
    >
      <div className="bg-muted/40 rounded-2xl px-4 py-3">
        <div
          className={`h-3.5 rounded-full overflow-hidden bg-background/30 ${width}`}
        >
