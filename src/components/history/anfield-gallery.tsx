...
export function AnfieldGallery() {
  ...
  const validCount = useCallback(
    (cat: Category) => {
      if (cat === "all") return IMAGES.reduce((acc, img) => acc + (!failedSrcs.has(img.src) ? 1 : 0), 0);
      return IMAGES.reduce((acc, img) => acc + (img.category === cat && !failedSrcs.has(img.src) ? 1 : 0), 0);
    },
    [failedSrcs],
  );
